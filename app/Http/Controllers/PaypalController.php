...
class PaypalController extends HomeController
{
    ...
    public function __construct()
    {
        ...
        $post = PaymentSetting::all();
        $clientId = "";
        $secret = "";
        $mode ="";
        foreach ($post as $paypal_conf) {
            $clientId = $paypal_conf->clientId;
            $secret = $paypal_conf->secretKey;
            $mode = $paypal_conf->paymetType;
    ...
    public function getPaymentStatus()
    {
        ...
        if ($result->getState() == 'approved') { 
            ...
            $post = array();
            $creditAmount = Session::get('amount');
            $newCredit = (int)$creditAmount / 1.67;

            $credit = User::select('credits')->where('id',Auth::id())->get();
            $oldCredit = 0;
            foreach ($credit as $value) {
                $oldCredit = (int)$value->credits;
            }
            $post['credits'] = $newCredit + $oldCredit;
            $i = User::where('id',Auth::id())->update($post); 
            return Redirect::route('paywithpaypal');
        }
        Session::put('error','Payment failed');
